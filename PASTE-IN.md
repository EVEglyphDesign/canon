...
# Operating canon — self-contained copy

Self-contained. Paste the whole of this file into a Perplexity Space, a
ChatGPT Project, or any surface that takes a single block of standing
instructions and cannot read this repository.

Because that surface cannot fetch the repository, nothing here links out.
When the canon changes, regenerate this file and re-paste it — the
provenance hash at the top is how you tell whether the copy in a Space is
current.
...
`OBSERVATIONS.md`. If the AI does not know what
...
- To log observations of drift in `OBSERVATIONS.md`
...
- To record changes to this file as Type-Δ rows in
  `OBSERVATIONS.md`, so the AI is not asked to
...
- To read this file first, then the current
  `OBSERVATIONS.md` (recent Type-Δ rows for what
...
- To act within the canon as it stands, and to state the ambiguity
  ...
  clearly fit a rule.
- To log its own drift in `OBSERVATIONS.md` when
...
`OBSERVATIONS.md` as a Type-O row in the same
